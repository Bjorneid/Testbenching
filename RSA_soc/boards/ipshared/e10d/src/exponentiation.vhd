    ...
    begin
    	if (rising_edge(clk)) then
			...
			case state is
				...
				-- We previously had a signal named r_sig, but this was swapped out with x_r.
				-- We try to make the circuit less complicated by reusing x_r
				-- Maybe we are wrong about this?
				when CALC_R =>
					reset_MP1 <= '0';
					
					-- x_r <= (x"047e4cdedbc494994fd35f61830eb71a09e440a5ac670359603191fd336943049")(C_block_size - 1 downto 0); --r_pow mod unsigned(modulus);	-- previously r_sig

					x_r <= 258x"047e4cdedbc494994fd35f61830eb71a09e440a5ac670359603191fd336943049";
			...
			-- the current x_r has a valid value, put the final result on the output bus.
			-- x_r has a size that is 2 bits larger than result
			if (ready_out = '1' and valid_out = '1') then
				result <= std_logic_vector(x_r(C_block_size-3 downto 0));
			end if;
        end if;
